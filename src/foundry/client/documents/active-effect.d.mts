...
declare namespace ActiveEffect {
  ...
  interface Schema extends fields.DataSchema {
     ...
     * @remarks Legacy v13 field names (`seconds`, `rounds`, `turns`, `startTime`, `startRound`,
     ...
     * @defaultValue `null`
     * @remarks Legacy non-UUID strings are migrated to `null` and copied to `flags.core.originText`
     * by {@linkcode BaseActiveEffect.migrateData}.
     */
   ...
   *
   * @remarks Legacy v13 field accesses are still readable via runtime shims that emit a
   * compatibility warning. They are typed here as optional so existing code continues to compile
   * while the warnings drive migration.
   */
   ...
   * `label`) plus internal flags (`_worldTime`, `_combatTime`). Legacy v13 readable properties
   * (`type`, `duration`) are installed at the same time via runtime getters that emit a
   ...
   * that maps it to v14's string `type` (`0` → `"custom"`, `1` → `"multiply"`, `2` → `"add"`,
  ...
  interface ChangeData {
    ...
    /**
     * The value of the change. v14 widened this to an `AnyField` so values may be any serializable
...
declare class ActiveEffect<out SubType extends ActiveEffect.SubType = ActiveEffect.SubType> extends BaseActiveEffect
   ...
   *
   * @deprecated since v14, will be removed in v16
   * @remarks Duration refreshes are now driven by {@linkcode ActiveEffectRegistry.refresh}; the
   * equivalent check happens inline.
   */
